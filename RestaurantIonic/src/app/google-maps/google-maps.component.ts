...
export class GoogleMapsComponent implements OnInit {
  ...
  constructor(private service: PlatsService, private utils: UtilsService, private renderer: Renderer2, private element: ElementRef, @Inject(DOCUMENT) private _document) { }
  ...
  private initMap(): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      ...
      Geolocation.getCurrentPosition().then((position) => {
        ...
        console.log("les restau");
        console.log(this.restaurants);
        this.restaurants.forEach(function(element) {
        GoogleMapsComponent.addMarker(element.latitude, (-1 * element.longitude), element.nom);
        })
        resolve(true);
      }, (err) => {
